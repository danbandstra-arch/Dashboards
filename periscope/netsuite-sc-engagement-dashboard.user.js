...
(function () {
  ...
  const HEAT_DISABLED_TABLES = new Set();
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .scd-table-tools .scd-definitions {
  ...
  function renderDashboard(summaryData, sourceCount, sourceLabel = "visible table", refreshedAt = new Date()) {
    ...
    function render() {
      ...
      root.innerHTML = `
        <div class="scd-modal-card" role="dialog" aria-modal="true" aria-label="${escapeHtml(CONFIG.title)}">
          ...
          <div class="scd-body">
            ...
            <div class="scd-panel scd-deep-grid">
              <div class="scd-panel-title">Geographic Staffing Volume</div>
              ${geographicStaffingTable(active)}
            </div>
      ...
      root.querySelectorAll("[data-scd-toggle-heat]").forEach((input) => {
        input.addEventListener("change", (event) => {
          const key = event.target.getAttribute("data-scd-toggle-heat");
          if (!key) return;
          if (event.target.checked) HEAT_DISABLED_TABLES.delete(key);
...
function rankedTable(map, label, limit = 10, denominator = 0) {
  ...
  function geographicStaffingTable(summary) {
    const entries = sortedEntries(summary.byBillingState);
    if (!entries.length) return `<div class="scd-warning">No Billing State/Province data found. Confirm Billing State/Province is included in the CSV export.</div>`;
    const total = summary.total || 1;
    const rows = entries.map(([billingState, volume]) => {
      const stateRows = summary.rows.filter((row) => normalizeText(row.billingState) === normalizeText(billingState));
      const consultants = new Set(stateRows.map((row) => row.consultant || "(blank)"));
      const amo = stateRows.filter((row) => salesMotionBucket(row) === "AMO").length;
      const direct = stateRows.filter((row) => salesMotionBucket(row) === "Direct").length;
      const channel = stateRows.filter((row) => salesMotionBucket(row) === "Channel").length;
      const drill = { billingState };
      return [
        { display: billingState, value: billingState, drill },
        { display: formatNumber(volume), value: volume, heat: true, drill },
        { display: `${((volume / total) * 100).toFixed(0)}%`, value: volume / total, heat: true, drill },
        { display: formatNumber(consultants.size), value: consultants.size, heat: true, drill },
        { display: formatNumber(amo), value: amo, heat: true, drill: { billingState, salesMotion: "AMO" } },
        { display: formatNumber(direct), value: direct, heat: true, drill: { billingState, salesMotion: "Direct" } },
        { display: formatNumber(channel), value: channel, heat: true, drill: { billingState, salesMotion: "Channel" } },
        { display: formatCurrency(stateRows.reduce((sum, row) => sum + pipelineRevenue(row), 0)), value: stateRows.reduce((sum, row) => sum + pipelineRevenue(row), 0), heat: true, drill: { billingState, oppBucket: "Open" } },
        { display: formatCurrency(stateRows.reduce((sum, row) => sum + closedRevenue(row), 0)), value: stateRows.reduce((sum, row) => sum + closedRevenue(row), 0), heat: true, drill: { billingState, oppBucket: "Won" } },
        { display: formatCurrency(stateRows.reduce((sum, row) => sum + rowRevenue(row), 0)), value: stateRows.reduce((sum, row) => sum + rowRevenue(row), 0), heat: true, drill },
        { display: formatCurrency(stateRows.reduce((sum, row) => sum + weightedRevenue(row), 0)), value: stateRows.reduce((sum, row) => sum + weightedRevenue(row), 0), heat: true, drill }
      ];
    });
    const footer = [
      { display: "Total", value: "Total" },
      ...
      { display: summary.total ? "100%" : "0%", value: summary.total ? 1 : 0 },
      { display: formatNumber(summary.consultants.size), value: summary.consultants.size },
      { display: formatNumber(summary.rows.filter((row) => salesMotionBucket(row) === "AMO").length), value: summary.rows.filter((row) => salesMotionBucket(row) === "AMO").length },
      { display: formatNumber(summary.rows.filter((row) => salesMotionBucket(row) === "Direct").length), value: summary.rows.filter((row) => salesMotionBucket(row) === "Direct").length },
      { display: formatNumber(summary.rows.filter((row) => salesMotionBucket(row) === "Channel").length), value: summary.rows.filter((row) => salesMotionBucket(row) === "Channel").length },
      { display: formatCurrency(summary.rows.reduce((sum, row) => sum + pipelineRevenue(row), 0)), value: summary.rows.reduce((sum, row) => sum + pipelineRevenue(row), 0) },
      { display: formatCurrency(summary.rows.reduce((sum, row) => sum + closedRevenue(row), 0)), value: summary.rows.reduce((sum, row) => sum + closedRevenue(row), 0) },
      { display: formatCurrency(summary.rows.reduce((sum, row) => sum + rowRevenue(row), 0)), value: summary.rows.reduce((sum, row) => sum + rowRevenue(row), 0) },
      { display: formatCurrency(summary.rows.reduce((sum, row) => sum + weightedRevenue(row), 0)), value: summary.rows.reduce((sum, row) => sum + weightedRevenue(row), 0) }
    ];
    return simpleTable(
      ["Billing State/Province", "Volume", "% of Shown", "Unique SCs", "AMO", "Direct", "Channel", "Pipeline Rev", "Closed Rev", "Revenue", "Weighted Rev"],
      rows,
      footer,
      { key: "geographic-staffing" }
    );
  }
  ...
  function simpleTable(headers, rows, footer = null, options = {}) {
    if (!rows.length) return `<div class="scd-warning">No rows found.</div>`;
    const tableKey = options.key || headers.join("|");
    const heatDisabled = HEAT_DISABLED_TABLES.has(tableKey);
